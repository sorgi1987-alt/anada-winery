...
function LotDrawer({ lot, product, supplier, transactions, onClose, onStatus, onAdjust, onTransfer, onDispose, onCorrectConsumption }: { lot: ProductLot; product?: ProductMaster; supplier?: Supplier; transactions: ProductStockTransaction[]; onClose: () => void; onStatus: (status: 'approved' | 'rejected' | 'recalled', notes: string) => void; onAdjust: (input: ProductStockAdjustmentInput) => void; onTransfer: (input: ProductLocationTransferInput) => void; onDispose: (input: ProductDisposalInput) => void; onCorrectConsumption: (input: ProductConsumptionCorrectionInput) => void }) {
  ...
  return <div className="drawer-scrim" onMouseDown={onClose}><aside className="supply-drawer" onMouseDown={(event) => event.stopPropagation()}><header><div><span className={`input-status ${effectiveProductLotStatus(lot)}`}>{t(statusKey[effectiveProductLotStatus(lot)])}</span><h2>{product?.name}</h2><p>{lot.code} · {lot.supplierLot}</p></div><button onClick={onClose} aria-label={t('common.close')}><X /></button></header><section className="lot-dossier"><div><small>{t('supplies.supplier')}</small><strong>{supplier?.name}</strong></div><div><small>{t('supplies.onHand')}</small><strong>{lot.quantityOnHand} {lot.unit}</strong></div><div><small>{t('supplies.location')}</small><strong>{lot.location}</strong></div><div><small>{t('supplies.certificate')}</small><strong>{lot.certificateRef || t('supplies.notLinked')}</strong></div></section><section className="lot-actions"><h3>Control de existencias</h3><div><button type="button" onClick={() => { const quantity=Number(prompt('Ajuste (+/-)')||0); const reason=prompt('Motivo')||''; onAdjust({ productLotId: lot.id, quantity, reason, performedAt:new Date().toISOString(), operator:'Elena Martín', notes:'' }) }}>Ajustar</button><button type="button" onClick={() => { const fromLocation=prompt('Ubicación origen', lot.location)||''; const toLocation=prompt('Ubicación destino')||''; const quantity=Number(prompt('Cantidad')||0); onTransfer({ productLotId:lot.id, fromLocation, toLocation, quantity, performedAt:new Date().toISOString(), operator:'Elena Martín', notes:'' }) }}>Transferir</button><button type="button" onClick={() => { const location=prompt('Ubicación', lot.location)||''; const quantity=Number(prompt('Cantidad')||0); const reason=prompt('Motivo de baja')||''; onDispose({ productLotId:lot.id, location, quantity, reason, performedAt:new Date().toISOString(), operator:'Elena Martín', notes:'' }) }}>Dar de baja</button></div></section><section><h3>{t('supplies.auditTrail')}</h3><div className="stock-timeline">{transactions.map((transaction) => <span key={transaction.id}><i><Check /></i><div><strong>{t(`supplies.transaction.${transaction.type}`)}</strong><small>{new Intl.DateTimeFormat(locale, { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(transaction.occurredAt))} · {transaction.operator}</small></div><em>{transaction.quantity ? `${transaction.quantity} ${transaction.unit}` : '—'}</em>{transaction.type === 'consumption' && transaction.status !== 'reversed' && <button type="button" onClick={() => { const reason=prompt('Motivo de corrección')||''; onCorrectConsumption({ transactionId:transaction.id, performedAt:new Date().toISOString(), operator:'Elena Martín', reason }) }}>Revertir</button>}</span>)}</div></section><label className="decision-notes"><span>{t('supplies.decisionNotes')}</span><textarea value={notes} onChange={(event) => setNotes(event.target.value)} placeholder={t('supplies.decisionPlaceholder')} /></label><footer>{effectiveProductLotStatus(lot) === 'quarantine' && <><button className="supply-danger-button" disabled={!notes.trim()} onClick={() => onStatus('rejected', notes)}>{t('supplies.reject')}</button><button className="primary-button" onClick={() => onStatus('approved', notes)}><Check /> {t('supplies.approve')}</button></>}{effectiveProductLotStatus(lot) === 'approved' && <button className="supply-danger-button full" disabled={!notes.trim()} onClick={() => onStatus('recalled', notes)}><ShieldAlert /> {t('supplies.recall')}</button>}</footer></aside></div>
}
