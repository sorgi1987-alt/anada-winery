...
function ReceiveLotSheet({ products, suppliers, onClose, onSave }: { products: ProductMaster[]; suppliers: Supplier[]; onClose: () => void; onSave: (input: NewProductLotInput) => void }) {
  ...
  return <div className="drawer-scrim" onMouseDown={onClose}><form className="supply-sheet" onMouseDown={(event) => event.stopPropagation()} onSubmit={submit}><header><div><span className="eyebrow">{t('supplies.receiptKicker')}</span><h2>{t('supplies.receiptTitle')}</h2><p>{t('supplies.receiptText')}</p></div><button type="button" onClick={onClose} aria-label={t('common.close')}><X /></button></header><div className="supply-form-grid">
    ...
    <label><span>{t('supplies.quantity')}</span><div><input required type="number" min="0.001" step="0.001" value={draft.quantity || ''} onChange={(event) => setDraft({ ...draft, quantity: Number(event.target.value) })} /><i>{draft.unit}</i></div></label>
  ...
  </div>{error && <p className="form-error"><AlertTriangle /> {error}</p>}<footer><button type="button" className="secondary-button" onClick={onClose}>{t('common.cancel')}</button><button className="primary-button"><PackageCheck /> {t('supplies.saveReceipt')}</button></footer></form></div>
}
