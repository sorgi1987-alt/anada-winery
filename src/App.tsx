...
const WhiteProcessControl = lazy(() => import('./WhiteProcess').then((module) => ({ default: module.WhiteProcessControl })))
const RoseProcessControl = lazy(() => import('./RoseProcess').then((module) => ({ default: module.RoseProcessControl })))
const MovementsPage = lazy(() => import('./Movements').then((module) => ({ default: module.MovementsPage })))
...
const navItems = [
  ...
  { labelKey: 'nav.movements' as const, path: '/movements', icon: ArrowRightLeft },
...
function App() {
  ...
  useEffect(() => {
    if (!authUser) return
    let cancelled = false
    fetchWineryContext().then(async (context) => {
      if (cancelled) return
      if (context.status !== 'unprovisioned' || !context.bootstrapAvailable) {
        setRemoteWineryContext(context)
        return
      }
      const winery = wineries.find((item) => item.id === currentWineryId)
      if (!winery) {
        setRemoteWineryContext(context)
        return
      }
      const provisioned = await provisionWinery({
        winery,
        campaigns,
        growers,
        vineyards,
    ...
    return () => { cancelled = true }
    // Deliberately narrow: this bootstraps at most once per login transition,
    // using whatever local state exists at that moment - it must not re-fire
  ...
  const saveReading = (lotId: string, reading: ReadingPoint, volume?: number) => {
    ...
    if ((targetLot?.type === 'tinto' || targetLot?.type === 'blanco' || targetLot?.type === 'rosado') && stage) {
  ...
  const saveRoseOperation = (input: NewRoseOperationInput) => {
    const result = recordRoseOperation(demoLots, demoTanks, tasks, productionEvents, input)
    setDemoLots(result.lots)
    setDemoTanks(result.tanks)
    setTasks(result.tasks)
    setProductionEvents(result.events)
  ...
  const moveRoseStage = (input: AdvanceRoseStageInput) => {
    const result = advanceRoseStage(demoLots, demoTanks, tasks, productionEvents, input)
    setDemoLots(result.lots)
    setDemoTanks(result.tanks)
    setTasks(result.tasks)
    setProductionEvents(result.events)
  ...
  const completeTransfer = (input: NewTransferInput) => {
    const result = transferWine(demoLots, demoTanks, movements, input)
    setDemoLots(result.lots)
    setDemoTanks(result.tanks)
  ...
  const completeSplit = (input: NewSplitInput) => {
    const result = splitWine(demoLots, demoTanks, movements, input)
    setDemoLots(result.lots)
    setDemoTanks(result.tanks)
    setMovements(result.movements)
    setUndoLot(null)
  ...
  const completeMerge = (input: NewMergeInput) => {
    const result = mergeWine(demoLots, demoTanks, movements, input)
    setDemoLots(result.lots)
    setDemoTanks(result.tanks)
    setMovements(result.movements)
    setUndoLot(null)
  ...
  const registerIntake = (input: NewGrapeIntakeInput) => {
    ...
    if (!activeCampaign) { showCampaignResult(locale.startsWith('es') ? 'Activa una campaña para registrar recepciones' : 'Activate a campaign before recording receptions'); return }
  ...
  const receiveInputLot = (input: NewProductLotInput) => {
    const result = receiveProductLot(input, productMasters, suppliers, productLots, productStockTransactions)
    setProductLots(result.lots)
    setProductStockTransactions(result.transactions)
    setToast(t('toast.productLotReceived', { code: result.lot.code }))
    window.setTimeout(() => setToast(null), 3200)
  }

  const addSupplier = (input: NewSupplierInput) => {
    const result = createSupplier(input, suppliers)
    setSuppliers(result.suppliers)
    setToast(t('toast.supplierCreated', { code: result.supplier.code }))
    window.setTimeout(() => setToast(null), 3200)
  }

  const addProductMaster = (input: NewProductMasterInput) => {
    const result = createProductMaster(input, productMasters)
    setProductMasters(result.products)
    setToast(t('toast.productCreated', { code: result.product.code }))
    window.setTimeout(() => setToast(null), 3200)
  }

  const updateInputLotStatus = (lotId: string, status: Extract<ProductLotStatus, 'approved' | 'rejected' | 'recalled'>, notes: string) => {
    const result = changeProductLotStatus(productLots, productStockTransactions, lotId, status, notes)
    setProductLots(result.lots)
    setProductStockTransactions(result.transactions)
    setToast(t('toast.productLotStatus', { code: result.lot.code }))
    window.setTimeout(() => setToast(null), 3200)
  }
  ...
  const runVineyardAction = (action: 'deactivate' | 'reactivate', id: string) => { try { const activeParcelCount = parcels.filter((p) => p.estateId === id && p.status !== 'inactive').length; setVineyards((current) => setVineyardStatus(current, id, action === 'deactivate' ? 'inactive' : 'active', masterOperator, activeParcelCount)); showCampaignResult(locale.startsWith('es') ? 'Estado del viñedo actualizado' : 'Vineyard status updated') } catch (error) { handleMasterError(error) } }
  const addParcel = (input: NewParcelInput) => { try { const result = createParcel(parcels, growers, vineyards, input); setParcels(result.parcels); if (result.plan) setCampaignParcels((current) => [...current, result.plan!]); showCampaignResult(locale.startsWith('es') ? 'Parcela creada' : 'Parcel created') } catch (error) { handleMasterError(error) } }
  const editParcel = (id: string, input: ParcelUpdateInput) => { try { setParcels((current) => updateParcel(current, growers, vineyards, id, input)); showCampaignResult(locale.startsWith('es') ? 'Parcela actualizada' : 'Parcel updated') } catch (error) { handleMasterError(error) } }
  const runParcelAction = (action: 'deactivate' | 'reactivate', id: string) => { setParcels((current) => setParcelStatus(current, id, action === 'deactivate' ? 'inactive' : 'active', masterOperator)); showCampaignResult(locale.startsWith('es') ? 'Estado de la parcela actualizado' : 'Parcel status updated') }
  const toggleParcelCampaign = (campaignId: string, parcelId: string, included: boolean) => { setCampaignParcels((current) => setParcelCampaignMembership(current, campaignId, parcelId, included, masterOperator)); showCampaignResult(locale.startsWith('es') ? (included ? 'Parcela añadida a la campaña' : 'Parcela retirada de la campaña') : (included ? 'Parcel added to campaign' : 'Parcel removed from campaign')) }
  ...
  const activeLots = demoLots.filter((lot) => lot.operationalStatus !== 'consumed')
  ...
  else if (pathname === '/admin/vineyards') currentPage = <AdministrationPage initialView="vineyards" settings={settings} campaigns={campaigns} growers={growers} vineyards={vineyards} parcels={parcels} campaignParcels={campaignParcels} lots={demoLots} deliveries={deliveries} bottlingOrders={bottlingOrders} recordCount={operationalRecordCount} pwa={pwa} onSave={saveWinerySettings} onCreateCampaign={addCampaign} onUpdateCampaign={editCampaign} onCampaignAction={runCampaignAction} onCreateGrower={addGrower} onUpdateGrower={editGrower} onGrowerAction={runGrowerAction} onCreateVineyard={addVineyard} onUpdateVineyard={editVineyard} onVineyardAction={runVineyardAction} onCreateParcel={addParcel} onUpdateParcel={editParcel} onParcelAction={runParcelAction} onToggleParcelCampaign={toggleParcelCampaign} onResetData={resetDemoData} remoteWineryContext={remoteWineryContext} />
  else if (pathname === '/admin/parcels') currentPage = <AdministrationPage initialView="parcels" settings={settings} campaigns={campaigns} growers={growers} vineyards={vineyards} parcels={parcels} campaignParcels={campaignParcels} lots={demoLots} deliveries={deliveries} bottlingOrders={bottlingOrders} recordCount={operationalRecordCount} pwa={pwa} onSave={saveWinerySettings} onCreateCampaign={addCampaign} onUpdateCampaign={editCampaign} onCampaignAction={runCampaignAction} onCreateGrower={addGrower} onUpdateGrower={editGrower} onGrowerAction={runGrowerAction} onCreateVineyard={addVineyard} onUpdateVineyard={editVineyard} onVineyardAction={runVineyardAction} onCreateParcel={addParcel} onUpdateParcel={editParcel} onParcelAction={runParcelAction} onToggleParcelCampaign={toggleParcelCampaign} onResetData={resetDemoData} remoteWineryContext={remoteWineryContext} />
  else if (pathname === '/admin/growers') currentPage = <AdministrationPage initialView="growers" settings={settings} campaigns={campaigns} growers={growers} vineyards={vineyards} parcels={parcels} campaignParcels={campaignParcels} lots={demoLots} deliveries={deliveries} bottlingOrders={bottlingOrders} recordCount={operationalRecordCount} pwa={pwa} onSave={saveWinerySettings} onCreateCampaign={addCampaign} onUpdateCampaign={editCampaign} onCampaignAction={runCampaignAction} onCreateGrower={addGrower} onUpdateGrower={editGrower} onGrowerAction={runGrowerAction} onCreateVineyard={addVineyard} onUpdateVineyard={editVineyard} onVineyardAction={runVineyardAction} onCreateParcel={addParcel} onUpdateParcel={editParcel} onParcelAction={runParcelAction} onToggleParcelCampaign={toggleParcelCampaign} onResetData={resetDemoData} remoteWineryContext={remoteWineryContext} />
  else if (pathname === '/admin/campaigns') currentPage = <AdministrationPage initialView="campaign" settings={settings} campaigns={campaigns} growers={growers} vineyards={vineyards} parcels={parcels} campaignParcels={campaignParcels} lots={demoLots} deliveries={deliveries} bottlingOrders={bottlingOrders} recordCount={operationalRecordCount} pwa={pwa} onSave={saveWinerySettings} onCreateCampaign={addCampaign} onUpdateCampaign={editCampaign} onCampaignAction={runCampaignAction} onCreateGrower={addGrower} onUpdateGrower={editGrower} onGrowerAction={runGrowerAction} onCreateVineyard={addVineyard} onUpdateVineyard={editVineyard} onVineyardAction={runVineyardAction} onCreateParcel={addParcel} onUpdateParcel={editParcel} onParcelAction={runParcelAction} onToggleParcelCampaign={toggleParcelCampaign} onResetData={resetDemoData} remoteWineryContext={remoteWineryContext} />
...
function LotDetail({ lots, tanks, productionEvents, productMasters, productLots, productTransactions, lotId, onReading, onConsumeProduct, onRecordRedOperation, onAdvanceRedStage, onRecordWhiteOperation, onAdvanceWhiteStage, onRecordRoseOperation, onAdvanceRoseStage }: {
  ...
  onRecordRoseOperation: (input: NewRoseOperationInput) => void
  onAdvanceRoseStage: (input: AdvanceRoseStageInput) => void
}) {
  const navigate = useNavigate()
  const { t, d, locale } = useLanguage()
