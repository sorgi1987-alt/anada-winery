...
Embedded authentication was enabled on the project 10 August 2026 (`embedded_auth: true`, `public_signup: false` — new accounts are provisioned via `Membership`/Catalyst project users, not open signup). The `anada_data_api` health contract was reconciled to schema v2 (17 tables) the same day and redeployed.

Built and deployed the same day:

- A real login screen (`src/Login.tsx`, `src/auth.ts`) using Catalyst's embedded Web SDK v4 (`catalystWebSDK.js` + the reserved `/__catalyst/sdk/init.js` project-config script, loaded on demand). Verified live on the deployed Slate domain — the real Catalyst-hosted sign-in iframe renders correctly inside Añada's branded shell. This cannot resolve from a local dev server; `/__catalyst/sdk/init.js` only exists on the Catalyst-connected Slate domain, so the component has a tested graceful-failure state (branded error message + retry) for that case.
- The app's entry point now gates on authentication: unauthenticated sessions see `Login`, not the operational app. `App.tsx` checks `catalyst.auth.isUserAuthenticated()` on load and polls the same check while the login form is shown (Zoho's SDK documents no explicit sign-in-succeeded callback).
...
- Provisioned the first real Catalyst App User (`sorgi1987@gmail.com`, App Administrator role) via `Add_User` so the login flow can be completed end-to-end by a human with real inbox access — the agent session that built this cannot complete Zoho's email confirmation step itself.
