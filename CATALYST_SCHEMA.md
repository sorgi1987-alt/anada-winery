...
`Anada_Tasks` (done and verified live, 11 August 2026): `TaskID`, `WineryID`, `LotID`, `Title`, `AssignedTo`, `TaskPriority`, `CompletionState`, `TaskTime` (new). `CellarTask.time` is a free-text display string (`'Hoy'`, `'16:00'`), not a real timestamp - the legacy `TaskDueAt` (`datetime`) column was dropped and replaced with a plain `varchar` column, not renamed in place, because `Time` itself is a reserved ZCQL keyword and Catalyst rejected it outright. Legacy `AppUpdatedAt` also dropped, same reasoning as `Anada_Tanks.UpdatedAt`. Verified live: marking a task complete through the real UI produced `CompletionState: true` with `TaskTime` preserved unchanged.
...
### `productionEvents` (new table, done and verified live, 11 August 2026)

`Anada_ProductionEvents` (new, 41 application columns) - `ProductionEvent`'s 14 core scalar fields plus its `metrics` object (`ProductionEventMetrics`, 27 optional scalar fields, no independent identity of its own) flattened directly onto the same table as individual columns, rather than a separate child table: `ProductionEventID`, `WineryID`, `LotID`, `WineType`, `Kind`, `StageID`, `OperationType`, `FromStageID`, `ToStageID`, `PerformedAt`, `RecordedAt`, `Operator`, `Notes`, `StorageMode`, then `DurationMinutes`, `Temperature`, `Density`, `VolumeBefore`, `VolumeAfter`, `FreeRunVolume`, `PressVolume`, `Product`, `ProductId`, `ProductLotId`, `SupplierLot`, `AdditionAmount`, `AdditionUnit`, `MalicAcid`, `FreeSo2`, `PotentialAlcohol`, `Turbidity`, `PressFraction`, `Protection`, `SettlingHours`, `LeesDecision`, `ConductivityDrop`, `ColorIntensity`, `SkinContactHours`, `RedGrapePercentage`, `SeparateWeightsConfirmed`, `MixingAfterWeighing`.

`backend/anada_data_api/wineryContext.js`'s `mapRow`/`toRow` gained small `getPath`/`setPath` helpers so a `TABLE_FIELDS` entry can use a dotted key (`'metrics.temperature'`) to read/write a nested browser-side field from/to a flat Catalyst column, fully backward compatible - every other table's plain, non-dotted keys behave identically to before. `src/wineryDiff.ts` gained a parallel `DEEP_FIELDS`/`deepEqualTolerant` mechanism (alongside the existing `DATETIME_FIELDS`): `ProductionEvent.metrics` stays a nested object on the browser side even though it's flattened remotely, so a freshly-pulled `metrics` object is never `===` its local counterpart even with identical content - without a tolerant recursive comparison (undefined/null/missing-key treated as equal at every level, same as the stage-3 tanks/tasks fix but one level deeper), every productionEvent would have looked "dirty" forever after any pull, the same self-retrigger failure class as the earlier null/undefined bug.

**A third real bug, found live**: ZCQL rejects any `SELECT` with more than 30 columns (`"More than 30 select columns are not allowed"`). `Anada_ProductionEvents` needs 41 app columns + `MODIFIEDTIME` = 42 in a single `queryRows` call - every table before it fit comfortably under the limit, so this was invisible until `productionEvents` was actually added to `WINERY_SCOPED_TABLES` and a real `GET /me/context` tried to read it. Because `queryRows`'s `SELECT` had no per-table guard, the thrown ZCQL error propagated out of `getContextForUser`'s scoped-table loop uncaught, taking down `GET /me/context` **for every winery-scoped table, not just `productionEvents`** - a live regression discovered by a `503` on the very first post-deploy read-back, then reproduced directly against Catalyst via `Execute_Query` (the same "trust the real Catalyst error, not the generic 503 catch-all" technique used for every prior Catalyst-specific bug this project has hit). Writes were unaffected (`insertRow`/`updateRow` go through the datastore API, not ZCQL, so `POST /me/sync` kept working the whole time - only reads broke). Fixed by teaching `queryRows` to split a wide table's columns into ZCQL-legal chunks (25 real columns per query, well under the 30-column ceiling) and re-join the resulting rows by `ROWID`, which every chunk shares since they're all the same `WHERE` over the same table; every other table still takes the original single-query path unchanged. Verified live: `GET /me/context` returned `200` again with all tables (including 9 real synced `productionEvents` covering red/white/rosé operation types, correctly reconstructing `metrics` from the flattened columns) after the fix deployed, where it had returned `503` immediately before.

**A fourth real bug, found live between `productionEvents` and `movements`**: `mergePulledRows` adopted a fresh remote row by replacing the whole local object with only the fields Catalyst tracks, silently dropping any required local-only field (`VineyardParcel.sample`/`.image` have no Catalyst column). A user hit this live - the Harvest page crashed reading `parcel.sample.potentialAlcohol` on `undefined` - almost certainly triggered by this session's own multi-tab sync testing being the first real trigger of a parcel merge since stage 2 shipped on 10 August. Fixed in `src/wineryDiff.ts` by spreading the local row first (`{ ...row, ...rest }`, not just `rest`), so every Catalyst-tracked field still adopts the remote value while local-only fields survive; `store.ts` also gained a one-time `repairParcelsMissingLocalOnlyFields` pass in `load()` to heal any install already left in the broken state. This fix turned out to matter again immediately for `movements`: a movement pulled fresh from another device has no `sourceLegs`/`destinationLegs` at all (not Catalyst columns), so without it a newly-pulled movement would render with no legs - the reattachment step below fills them back in regardless, but the underlying merge fix is what keeps a movement's legs from being wiped on any *other* field's remote change in between reattachment passes.

### `movements` + `movementLegs` (new tables, done and verified live, 12 August 2026) - first child-table sync

`Anada_WineMovements`: `MovementID`(mandatory,unique), `WineryID`(mandatory), `Code`, `Kind`, `WineType`, `GrossSourceVolume`, `ReceivedVolume`, `LossVolume`, `LossPercentage`, `PerformedAt`, `RecordedAt`, `Operator`, `Notes`, `StorageMode`. `WineMovement.sourceLegs`/`.destinationLegs` are **not** columns here - `WineMovementLeg` has no identity of its own in `src/types.ts` (a plain value object), so legs get their own child table instead of being flattened, per the user's earlier "child tables for append-only/identified data" decision.

`Anada_MovementLegs`: `LegID`(mandatory,unique), `WineryID`(mandatory, denormalized from the parent movement - see below), `MovementID`(mandatory), `Side`(mandatory, `'source'|'destination'`), `Sequence`(int - required, since ZCQL gives no `ORDER BY` guarantee and ordering within a side matters), `LotID`, `LotName`, `VesselID`, `VolumeBefore`, `MovementVolume`, `VolumeAfter`.

Movement legs have no identity or persisted state of their own on the browser side - `App.tsx`'s `deriveMovementLegs` synthesizes one fresh from `movements` on every sync tick (`id: '${movementId}-${side}-${index}'`, safe as an index-based id here specifically *because* a movement and its legs are created atomically once and never revisited afterward - unlike readings, which append repeatedly and will need a non-index-based id when that collection is wired up). The reverse direction, `reattachMovementLegs`, rebuilds each movement's `sourceLegs`/`destinationLegs` from the merged legs collection after every sync tick, using a new generic `groupSortedBy` helper (`src/wineryDiff.ts`) to group legs by `movementId+side` and order each group by its own `sequence` field rather than array position. Both directions reuse `deepEqualTolerant` (exported from the same module) to preserve a movement's exact object reference when its legs didn't actually change, keeping the same reference-stability contract `mergePulledRows` itself relies on to avoid a self-retrigger loop. Every synced leg carries its own denormalized `WineryID` (copied from the parent movement), so `syncTable`'s existing `wineryIds.includes(row.wineryId)` authorization check works unchanged for the child table - no child-table-specific authorization code was needed.

Verified live: recorded a real "Trasiego completo" transfer through the actual UI (T-26-017, D-12 → D-21, 7850 L), confirmed the app's own natural sync loop pushed the movement and both legs to Catalyst correctly (source/destination, sequence, volumes all exact), then specifically exercised the reattachment path by clearing the movement from local state and loading a fresh tab - it came back with `sourceLegs`/`destinationLegs` correctly reconstructed from the independently-synced legs collection, not just revived from a stale local copy. Test data removed after verification.

### `lots` + `readings` + `activities` (rebuilt/done and verified live, 12 August 2026) - last and most complex slice of Batch 1

`Anada_WineLots` (rebuilt in place): `LotID`, `WineryID`, `Name`, `WineType`, `Varieties`, `Origin`, `Vintage`, `VolumeLitres`, `VesselID`, `Stage`, `Progress`, `Attention`, `NextAction`, `NextTime`, `ProcessJSON`, `ProductionJSON`, plus new columns `Day`, `TemperatureC`, `Density`, `AttentionText`, `OperationalStatus`, `CampaignID`, `CurrentVesselID`; legacy `UpdatedAt` dropped (no audit quartet, same reasoning as every other stage-3 table). `WineLot.image`/`.readings`/`.activities` are deliberately not columns - `image` is a Vite-bundled local asset path, not portable data (a `url(undefined)` background-image degrades silently, no crash, so no UI fix was needed); `readings`/`activities` sync as their own child tables instead, below.

A new `'json'` wireType (`backend/anada_data_api/wineryContext.js`) stores `WineLot.process`/`.productionDetails` as a `JSON.stringify`'d blob in a plain text column, since Catalyst has no native JSON type - `toRow` stringifies on write, `mapRow`'s `coerce.json` parses on read (tolerating a malformed/empty value as `null`, same as every other coerce function). `src/wineryDiff.ts`'s existing `DEEP_FIELDS`/`deepEqualTolerant` mechanism (built for `productionEvents.metrics`) was extended to `lots: ['process', 'productionDetails']` rather than building a second, less robust JSON.stringify-equality mechanism - `process`/`productionDetails` stay real arrays/objects on the browser side even though they're flattened to text remotely, so a freshly-parsed value is never `===` its local counterpart even with identical content.

`Anada_Readings` (rebuilt in place): `ReadingID` (resized `varchar(80)` → `varchar(100)`, since the synthesized id `${lotId}::${recordedAt}` can approach ~76 characters), `WineryID`, `LotID`, `RecordedAt`, `TemperatureC`, `Density`, `VolumeLitres`, `Note`; legacy `Label` dropped. `Anada_Activities` needed no column changes at all - it already matched the target shape exactly.

`ReadingPoint` has no identity of its own in `src/types.ts` - `App.tsx`'s `deriveReadings` synthesizes `${lotId}::${recordedAt}`, deliberately **not** index-based (unlike movement legs): readings append repeatedly, potentially from multiple devices with diverged local array lengths, so an index could collide across genuinely different readings where a movement's legs (created once, atomically, never revisited) safely can't. `LotActivity` does have a real `id`, so no synthesis is needed there. Neither collection is promoted to a top-level `WineryState` field - both are sync-only derived projections computed fresh from `demoLots` every tick and reattached via a new `reattachLotChildren` (readings ordered oldest-first to match the fermentation chart and how `domain.ts` appends them; activities newest-first, since `domain.ts` always prepends). `time` (a display string like `'Ahora'`) is deliberately not a column on either table - it's reconstructed via `formatZonedDateTime` (a fixed clock-time string, not a relative one, so the reconstructed value doesn't keep drifting and making an otherwise-unchanged lot look "dirty" forever) whenever a reading/activity arrives from another device with no local counterpart to preserve the original value from.

Verified live end-to-end, including cleanup of two rounds of self-inflicted test-data drift this step surfaced: recording a real reading through the UI confirmed `process`/readings/activities all synced correctly (direct Catalyst read-back matched exactly), and the reattachment path was confirmed the same way as movements - clear locally, load a fresh tab, confirm reconstruction. Separately, this step's first live sync of `lots` surfaced that an earlier session step's cleanup had been incomplete: a real "Trasiego completo" transfer recorded while verifying `movements` had genuinely moved T-26-017's wine from D-12 to D-21 (a real domain-level side effect of a real UI action, not corrupted data) - only the resulting `Anada_WineMovements`/`Anada_MovementLegs` test rows had been deleted afterward, not the lot's own vessel assignment, which had no sync path to reveal it until now. Fixed with a second real reverse transfer through the UI (the correct way to undo a transfer, since raw state edits would bypass `domain.ts`'s own volume/loss reconciliation) rather than direct state surgery, then the same test-row cleanup applied to both transfers.

### Batch 1 wrap-up

`backend/anada_data_api/contract.js`'s `SCHEMA_VERSION` bumped 2 → 3 and `TABLES` grew from 17 to 20 with the three tables this batch created (`Anada_ProductionEvents`, `Anada_WineMovements`, `Anada_MovementLegs` - the five rebuilt-in-place legacy tables, `Anada_WineLots`/`Anada_Tanks`/`Anada_Tasks`/`Anada_Readings`/`Anada_Activities`, were already present in the contract from earlier phases). `contract.test.js` updated to match. Batch 1 of Phase 9.5 stage 3 is now complete: `lots`, `tanks`, `tasks`, `productionEvents`, `movements` (plus the `readings`/`activities`/`movementLegs` child collections) all sync live. Batch 2 - the remaining ~20 browser-local collections (lab, barrels, blending, bottling, traceability, supplies, weather) - is a separate, deliberately deferred future effort; see the "Context" note at the top of the batch's original plan for why it was scoped this way.

## Phase 9.5 stage 3, Batch 2 (in progress, started 12 August 2026) - `deliveries`

First slice of Batch 2: `Anada_Deliveries` (new table, 23 application columns) - `GrapeDelivery`'s full field set: `DeliveryID`, `WineryID`, `Code`, `ParcelID`, `Grower`, `Varieties`, `Origin`, `ScheduledDate`, `ScheduledTime`, `ExpectedKg`, `DeliveryStatus`, `Vehicle`, `ProcessingDestination`, `ReceivedAt`, `GrossKg`, `TareKg`, `NetKg`, `Temperature`, `PotentialAlcohol`, `GrapeCondition`, `Notes`, `GrowerID`, `CampaignID`. `ScheduledDate`/`ScheduledTime` are plain form strings (`'2026-09-17'`, `'09:00'`), not full ISO timestamps, and stay `string` wireType, not `datetime` - only `ReceivedAt` (`new Date().toISOString()` in `domain.ts`) is a real timestamp. `Status`/`Condition` were named `DeliveryStatus`/`GrapeCondition` rather than the bare `Status`/`Condition` used elsewhere, purely to avoid re-testing whether either is a reserved ZCQL keyword given time constraints - not because either was confirmed reserved.

`vineyardSamples` (`VineyardSampleRecord[]`), also nominally part of Batch 2's "Harvest" grouping, turned out on inspection to be a pure derived/computed projection - `buildCanonicalRelationshipModel` (`relationships.ts`) rebuilds it entirely from `VineyardParcel.sample` on every relationship-model recompute, with no independent create/edit path anywhere in the app (`App.tsx` doesn't even winery-scope it with `useWineryScopedState`, unlike every other synced collection). Since `VineyardParcel.sample` is itself local-only and unsynced by design, syncing `vineyardSamples` separately would sync a value that's fully reconstructible from already-local data and never independently edited - deliberately skipped rather than adding a table with no real information content of its own.

`backend/anada_data_api/contract.js`'s `SCHEMA_VERSION` bumped 3 → 4, `TABLES` grew from 20 to 21. Verified live: all 5 real demo deliveries (mixed `planned`/`en_route`/`at_gate`/`received` statuses) synced correctly via the app's own `/me/sync` push, including `null` handling for the optional fields only a `received` delivery has (`netKg`, `condition`) - no test data was created for this verification, so no cleanup was needed.

## Phase 9.5 stage 3, Batch 2 continued (12 August 2026) - `samples` (Lab slice)

`Anada_LabSamples` (new table, 18 application columns): `SampleID`, `WineryID`, `Code`, `SourceType`, `SourceID`, `SourceName`, `WineType`, `Profile`, `CollectedAt`, `CollectedBy`, `AssignedTo`, `DueAt`, `SamplePriority`, `SampleStatus`, `RequestedAnalysesJSON`, `ResultsJSON`, `Notes`, `ValidatedAt`. `LabSample.requestedAnalyses`/`.results` are arrays with no independent identity, flattened through the `'json'` wireType (same treatment as `ProductionEvent.metrics`/`WineLot.process`) rather than given a child table. `DueAt` is a plain `<input type="time">` value (`'17:00'`), not a real datetime - only `CollectedAt`/`ValidatedAt` (both `new Date().toISOString()` in `domain.ts`) are real timestamps.

**Another reserved ZCQL keyword found live**: `Create_Column` rejected `Priority` outright (`"Column name cannot contain reserved keywords"`), the same class of error `Time` produced for `Anada_Tasks` in stage 3's first batch. Renamed to `SamplePriority`; `SampleStatus` follows the same naming for consistency, though plain `Status` has worked without issue on every other table in this schema.

`src/wineryDiff.ts` extended `DEEP_FIELDS.samples = ['requestedAnalyses', 'results']`, reusing the same deep-equality mechanism as `lots.process`/`productionEvents.metrics` rather than adding a third JSON-equality implementation.

`SCHEMA_VERSION` bumped 4 → 5, `TABLES` grew to 22. Verified live: all 5 real demo lab samples (spanning every status - `queued`, `in_analysis`, `review`, `validated` - and profile) synced correctly via the app's own `/me/sync` push, with `requestedAnalyses`/`results` round-tripping exactly including populated result arrays with real values/units/statuses. No test data was created for this verification either.

## Phase 9.5 stage 3, Batch 2 continued (14 August 2026) - `barrels` + `barrelOperations`

`Anada_Barrels` (new table, 22 application columns): `Barrel`'s full field set - `BarrelID`, `WineryID`, `Code`, `Cooperage`, `OakOrigin`, `Toast`, `Grain`, `Capacity`, `VolumeLitres`, `Status`, `Room`, `Rack`, `Position`, `UseNumber`, `LotID`, `LotName`, `WineType`, `FilledAt`, `PlannedMonths`, `Attention`, `NextAction`, `NextDue`, `Notes`. `FilledAt` is a **date-only** string in `domain.ts` (`new Date().toISOString().slice(0, 10)`, e.g. `'2026-08-01'`, no time component) but still uses the `'datetime'` wireType rather than a plain varchar - `toCatalystDatetime`/`fromCatalystDatetime` on the server and `truncateToSeconds` on the client both normalize a date-only value and a full ISO timestamp to the same `"yyyy-MM-ddT00:00:00"` representation, so it round-trips correctly without any special-casing (confirmed both in a backend unit test and live). `NextDue` is a free-text display string (`'7 días'`, `'2 oct'`, `'Hoy · 16:00'`), not a real timestamp - plain varchar, same treatment as `CellarTask.time`.

`Anada_BarrelOperations` (new table, 8 application columns): `OperationID`, `WineryID`, `OperationType`, `BarrelIDsJSON`, `TargetLabel`, `PerformedAt`, `Person`, `VolumeAdded`, `Notes`. `BarrelOperation.barrelIds` is an array with no independent identity - an operation can target several barrels at once (e.g. a batch racking) - flattened via the `'json'` wireType and `DEEP_FIELDS.barrelOperations = ['barrelIds']`, same treatment as `WineLot.process`/`LabSample.requestedAnalyses`.

`SCHEMA_VERSION` bumped 5 → 6, `TABLES` grew to 24. Verified live: all 18 real demo barrels and 4 real demo barrel operations synced correctly via the app's own `/me/sync` push - including the date-only `FilledAt` round-trip and a `NextDue` value containing a time component (`'Hoy · 16:00'`). No test data was created for this verification. `blending`/`bottling`/`traceability`/`supplies`/`weather` are still not started.
