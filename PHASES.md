...
**Stage 3 (Batch 1 complete)**: extended sync to the app's core cellar-operations collections (lots, tanks, tasks, production events, movements) one table at a time, verifying each live, 10–12 August 2026. `tanks`, `tasks`, `productionEvents`, `movements`/`movementLegs`, and `lots`/`readings`/`activities` are all done and verified live — see `CATALYST_SCHEMA.md`'s "Phase 9.5 stage 3" section, which also documents every bug found while verifying this batch live: an `undefined`-vs-`null` equality gap in the sync-diff logic that had likely been causing a silent, continuous self-retrigger loop since stage 2 shipped (not tanks-specific — fixed retroactively for all synced collections); two Phase 9.3 ID columns (`Anada_Vineyards.VineyardID`, `Anada_CampaignParcelPlans.PlanID`) that were too short for real generated ids, silently truncated during the original stage 1 bootstrap; ZCQL's undocumented 30-column `SELECT` limit, which `Anada_ProductionEvents`'s 41 columns exceeded and which broke `GET /me/context` for every winery-scoped table (not just productionEvents) until `queryRows` learned to chunk wide-table reads and re-join by `ROWID`; and a sync-merge bug that replaced a whole local row with only Catalyst-tracked fields on adoption, silently dropping required local-only fields (`VineyardParcel.sample`/`.image`) and crashing the Harvest page for a real user — fixed by preserving local-only fields during merge. `movements` was this batch's first child-table sync (`movementLegs`, synthesized ids, `groupSortedBy`-based reattachment of a movement's legs after every pull); `lots` added a `'json'` wireType for `process`/`productionDetails` and reused the same deep-equality mechanism built for `productionEvents.metrics`, plus a second reattachment pass for `readings`/`activities` (synthesized non-index-based ids for readings specifically, since they append repeatedly unlike a movement's legs). Batch 2 (the remaining ~20 browser-local collections: lab, barrels, blending, bottling, traceability, supplies, weather) is a separate, deliberately deferred future effort.
