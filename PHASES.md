...
**Stage 3 (in progress)**: extends sync to the app's core cellar-operations collections (lots, tanks, tasks, production events, movements) one table at a time, verifying each live. `tanks`, `tasks` and `productionEvents` done and verified live (10–11 August 2026) — see `CATALYST_SCHEMA.md`'s "Phase 9.5 stage 3" section, which also documents three bugs found while verifying this batch live: an `undefined`-vs-`null` equality gap in the sync-diff logic that had likely been causing a silent, continuous self-retrigger loop since stage 2 shipped (not tanks-specific — fixed retroactively for all synced collections); two Phase 9.3 ID columns (`Anada_Vineyards.VineyardID`, `Anada_CampaignParcelPlans.PlanID`) that were too short for real generated ids, silently truncated during the original stage 1 bootstrap; and ZCQL's undocumented 30-column `SELECT` limit, which `Anada_ProductionEvents`'s 41 columns exceeded and which broke `GET /me/context` for every winery-scoped table (not just productionEvents) until `queryRows` learned to chunk wide-table reads and re-join by `ROWID`. All three fixed live. `movements`/`lots` not yet started.
