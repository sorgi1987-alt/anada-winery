...
Phase 9.1 is the version 0.37.0 checkpoint. `Winery`, `User` and `Membership` exist as real domain entities with stable IDs; every operational collection carries a `wineryId`; browser schema v27 derives a default winery/user/membership from the existing single-winery data without regenerating any record's identity. This is foundational only: no page filters by winery yet (Phase 9.2), no Catalyst Schema v2 tables are provisioned for any of this (Phase 9.3), and authentication remains deferred (Phase 9.4). Phase 9B.1 (version 0.36.0) made vessel usable capacity configurable independently of nominal capacity, enforced in new-lot assignment, transfers, splits and merges. Phase 10A.3 (version 0.35.0) made vineyard parcels permanent, campaign-independent master data under Administration. Phase 10A.2 (version 0.34.0) made growers permanent master data. Campaign lifecycle (Phase 10A.1) shipped in version 0.33.0 with a full Administration workspace.
