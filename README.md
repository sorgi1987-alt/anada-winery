...
Version 0.35.0 makes vineyard parcels permanent, campaign-independent master data: `Grower -> Vineyard -> Parcel` under Administration (`/admin/vineyards`, `/admin/parcels`), with campaign-specific planning (expected yield, harvest window, readiness) held separately in `CampaignParcelPlan` records. Browser schema v26 derives these from every existing parcel's campaign reference without regenerating parcel IDs or grower/location relationships. Harvest now scopes to the active campaign and disables intake when none is active. Version 0.34.0 added canonical grower master management (legal/trade identity, contact details, active/inactive lifecycle, duplicate code/tax-ID validation, schema v25). Campaign lifecycle (create, activate, close, reopen, archive, single-active/default invariants, closure blockers) shipped in version 0.33.0 with a full Administration workspace at `/admin/campaigns`. The former DOCa Rioja eligibility workspace remains retired.
