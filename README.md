...
Version 0.38.0 enforces winery scoping end to end: every page reads a winery-filtered view and every mutation is stamped with the active winery, switching wineries in the sidebar is functional, and a second demo winery proves isolation — a record created while scoped to one winery is verified absent from the other. Version 0.37.0 introduced `Winery`, `User` and `Membership` as real domain entities. Neither touches Catalyst yet: no tables exist for any of this, and authentication remains deferred (Phase 9.3–9.5). Version 0.36.0 closed out cellar asset normalization: vessel usable capacity is configurable below nominal capacity, enforced in new-lot assignment, transfers, splits and merges. Version 0.35.0 made vineyard parcels permanent, campaign-independent master data under Administration. Version 0.34.0 added canonical grower master management. Campaign lifecycle shipped in version 0.33.0. The former DOCa Rioja eligibility workspace remains retired.
