...
Version 0.37.0 begins Phase 9 (shared production infrastructure): `Winery`, `User` and `Membership` now exist as real domain entities, and every operational collection carries a `wineryId`. This is foundational only — the app still runs against a single derived default winery and no page filters by it yet; scoping enforcement, real Catalyst table provisioning and authentication are separate, later sub-phases (9.2–9.5). Version 0.36.0 closed out cellar asset normalization: a vessel's usable capacity can be configured below its nominal capacity from the cellar map, and new-lot assignment, transfers, splits and merges all validate against usable capacity rather than nominal capacity. Version 0.35.0 made vineyard parcels permanent, campaign-independent master data under Administration (`/admin/vineyards`, `/admin/parcels`), with campaign-specific planning held separately in `CampaignParcelPlan` records. Version 0.34.0 added canonical grower master management. Campaign lifecycle shipped in version 0.33.0 with a full Administration workspace. The former DOCa Rioja eligibility workspace remains retired.

The current frontend remains a browser-authoritative prototype. Catalyst confirms the seven provisioned Development tables through a health-only function, but those tables predate campaigns, growers, vineyards, parcels, vessels and the new winery/user model entirely; shared server persistence, authentication, protected operational reads, remote writes and synchronization are not active.
